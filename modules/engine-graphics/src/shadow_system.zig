...
pub const ShadowSystem = struct {
    ...
    pub fn beginPass(self: *ShadowSystem, command_buffer: c.VkCommandBuffer, cascade_index: u32, light_space_matrix: Mat4) void {
        ...
        if (self.shadow_render_pass == null) {
            @import("engine-core").log.log.err("ShadowSystem: cannot begin pass, shadow_render_pass is null", .{});
            return;
        }
        if (cascade_index >= rhi.SHADOW_CASCADE_COUNT) {
            @import("engine-core").log.log.err("ShadowSystem: cascade index {} out of bounds", .{cascade_index});
            return;
        }
        if (self.shadow_framebuffers[cascade_index] == null) {
            @import("engine-core").log.log.err("ShadowSystem: framebuffer for cascade {} is null", .{cascade_index});
            return;
        }
        if (command_buffer == null) {
            @import("engine-core").log.log.err("ShadowSystem: cannot begin pass, command_buffer is null", .{});
            return;
        }
