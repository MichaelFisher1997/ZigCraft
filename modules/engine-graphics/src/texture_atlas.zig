...
const FilterMode = @import("engine-rhi").FilterMode;
...
pub const BlockTextureDefinition = struct {
    id: u8,
    name: []const u8,
    default_color: [3]f32,
...
pub const TextureAtlas = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, resources: rhi.ResourceManager, pack_manager: ?*resource_pack.ResourcePackManager, max_resolution: u32, block_textures: []const BlockTextureDefinition) !TextureAtlas {
        ...
        const tile_size = detectTileSize(pack_manager, allocator, max_resolution, block_textures);
...
fn loadBlockTextures(
    ...
    atlas_size: u32,
    stats: *LoadStats,
    block_textures: []const BlockTextureDefinition,
) !void {
...
fn detectTileSize(pack_manager: ?*resource_pack.ResourcePackManager, allocator: std.mem.Allocator, max_resolution: u32, block_textures: []const BlockTextureDefinition) u32 {
    if (pack_manager) |pm| {
        if (pm.getActivePackPath()) |pack_path| {
            const uses_pbr = pm.hasPBRSupport();

            for (block_textures) |config| {
