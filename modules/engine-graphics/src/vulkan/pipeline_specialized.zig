...
/// A vertex-pulling pipeline intentionally has no Vk vertex bindings.  Indexed
/// grid topology reaches the compact samples through descriptor binding 16.
pub fn createCompactLODPipeline(
    self: anytype,
    allocator: std.mem.Allocator,
    vk_device: c.VkDevice,
    ...
    const shaders = try loadShaderPair(allocator, vk_device, vert_path, frag_path);
    ...
    var compact_rasterizer = rasterizer.*;
    // Edge skirts contain both orientations around the tile perimeter. They
    // must remain visible regardless of winding after Vulkan Y inversion.
    compact_rasterizer.cullMode = c.VK_CULL_MODE_NONE;
    compact_rasterizer.polygonMode = c.VK_POLYGON_MODE_FILL;
    var water_blend_attachment = color_blending.pAttachments[0];
    var water_color_blending = color_blending.*;
    if (water) {
        water_depth_stencil.depthWriteEnable = c.VK_FALSE;
        water_blend_attachment.blendEnable = c.VK_TRUE;
        water_blend_attachment.srcColorBlendFactor = c.VK_BLEND_FACTOR_SRC_ALPHA;
        water_blend_attachment.dstColorBlendFactor = c.VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
        water_blend_attachment.colorBlendOp = c.VK_BLEND_OP_ADD;
        water_blend_attachment.srcAlphaBlendFactor = c.VK_BLEND_FACTOR_ONE;
        water_blend_attachment.dstAlphaBlendFactor = c.VK_BLEND_FACTOR_ZERO;
        water_blend_attachment.alphaBlendOp = c.VK_BLEND_OP_ADD;
        water_color_blending.pAttachments = &water_blend_attachment;
    }
    ...
    info.pRasterizationState = &compact_rasterizer;
    info.pMultisampleState = multisampling;
    info.pDepthStencilState = if (water) &water_depth_stencil else depth_stencil;
    info.pColorBlendState = if (water) &water_color_blending else color_blending;
    info.pDynamicState = dynamic_state;
    info.layout = self.pipeline_layout;
    info.renderPass = render_pass;
    info.subpass = 0;
    try Utils.checkVk(c.vkCreateGraphicsPipelines(vk_device, null, 1, &info, null, out_pipeline));
}
